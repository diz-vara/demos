...
if paths.filep('trainSgC12.t7') 
   and paths.filep('testSgC12.t7') then

   print(sys.COLORS.red ..  '==> loading previously generated dataset:')
   trainData = torch.load('trainSgC12.t7')
   testData = torch.load('testSgC12.t7')
   ...
   local desImaX = 12 -- desired cropped dataset image size
   local desImaY = 12
   ...
   if paths.filep('trainSgC12.t7') then
      ...
      torch.save('trainSgC12.t7',trainData)
   ...
   if paths.filep('testSgC12.t7') then
      ...
      torch.save('testSgC12.t7',testData)
   end
end
