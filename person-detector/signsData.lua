...
if paths.filep('trainSg.t7') 
   and paths.filep('testSg.t7') then

   print(sys.COLORS.red ..  '==> loading previously generated dataset:')
   trainData = torch.load('trainSg.t7')
   testData = torch.load('testSg.t7')
   ...
   if paths.filep('trainSg.t7') then
      ...
      for i = 1, trSize, 2 do
         ...
         img = image.loadPNG(trainDir..ls(trainDir)[(i-1)/2+1],ivch) -- we pick all of the images in train!
            trainData.data[trShuffle[i]] = img:clone() --.crop(img, cropTrX-desImaX/2, cropTrY-desImaY/2, cropTrX+desImaX/2, cropTrY+desImaY/2):clone()
      ...
      torch.save('trainSg.t7',trainData)
   ...
   if paths.filep('testSg.t7') then
      ...
      for i = 1, teSize, 2 do
         ...
         testData.data[i] = img:clone(); --image.crop(img, cropTeX-desImaX/2, cropTeY-desImaY/2, cropTeX+desImaX/2, cropTeY+desImaY/2):clone()
